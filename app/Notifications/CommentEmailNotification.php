...
class CommentEmailNotification extends Notification
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
                    ->subject('ADEMC Support Services - Update on Ticket #'.$this->comment->ticket->id)
                    ->greeting('Hi ADEMC Member,')
                    ->line('New update on ADEMC Support Ticket # '.$this->comment->ticket->id)
                    ->line('Ticket Subject: '.$this->comment->ticket->title)
                    ->line('')
                    ->line('Update:')
